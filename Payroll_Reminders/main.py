...
def send_slack_dm(user_id, message, hours_logged, hours_expected):
    ...
    try:
        slack.chat_postMessage(
            ...
            text=message,
            blocks=[
                {"type": "header", "text": {"type": "plain_text", "text": "⏰ Timesheet Reminder"}},
                {"type": "section", "text": {"type": "mrkdwn", "text": message}},
                {
                    "type": "section",
                    "fields": [
                        {"type": "mrkdwn", "text": f"*Hours Logged:*\n{hours_logged:.1f} hours"},
...
def send_reminders(dry_run=False, days=7, min_hours=32):
    ...
    for user in missing:
        ...
        if slack_user_id:
            success = send_slack_dm(slack_user_id, message, user['hours_logged'], user['hours_expected'])
            if success:
                sent_count += 1
            else:
                failed_count += 1
